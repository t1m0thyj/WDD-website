...
function loadThumbnailGrid(themeType, pageNumber) {
    ...
    isLoading = false;
    if (previewThemeId && themesDb[previewThemeId] && themesDb[previewThemeId].themeType === themeType) {
...
function openPreview(themeId) {
    ...
    $('#downloadButton').html('<i class="fa fa-download"></i> Download (' + themesDb[themeId].fileSize.toString() + ' MB)');
    $('#downloadButton').attr('href', themesDb[themeId].themeUrl);
